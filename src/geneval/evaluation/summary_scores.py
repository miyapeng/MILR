...
with open(output_path, "w", encoding="utf-8") as f:
    ...
    print("=======", file=f)
    print(f"Total images: {len(df)}", file=f)
    print(f"Total prompts: {len(df.groupby('metadata'))}", file=f)
    print(f"% correct images: {df['correct'].mean():.2%}", file=f)
    print(f"% correct prompts: {df.groupby('metadata')['correct'].any().mean():.2%}", file=f)
    print("", file=f)

    # Task breakdown
    task_scores = []
    print("Task breakdown", file=f)
    print("==============", file=f)
    for tag, task_df in df.groupby('tag', sort=False):
        score = task_df['correct'].mean()
        task_scores.append(score)
        print(f"{tag:<16} = {score:.2%} ({task_df['correct'].sum()} / {len(task_df)})", file=f)
    print("", file=f)

    print(f"Overall score (avg. over tasks): {np.mean(task_scores):.5f}", file=f)

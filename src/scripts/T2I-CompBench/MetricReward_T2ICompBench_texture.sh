...
# output_dir="./T2ICompBench_results/seed41/texture"
...
# # === 启动训练脚本 ===
# TOKENIZERS_PARALLELISM=false CUDA_VISIBLE_DEVICES=2 python main_janus.py \
...
# === 启动训练脚本 ===
TOKENIZERS_PARALLELISM=false CUDA_VISIBLE_DEVICES=5 python main_janus.py \
    --dataset "$PATH_TO_DATA" \
    --model_name_or_path "$PATH_TO_MODEL" \
    --output_dir "$output_dir" \
    --data_name "$data_name" \

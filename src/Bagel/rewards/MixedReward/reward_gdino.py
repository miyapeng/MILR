...
# 修改为相对导入（适配 GroundingDINO 放在本地文件夹里）
import sys
import os
# 添加 GroundingDINO 路径（确保可以 import groundingdino）
GROUNDING_DINO_ROOT = os.path.join(os.path.dirname(__file__), "GroundingDINO")
sys.path.append(GROUNDING_DINO_ROOT)
...
class GDino:
    def __init__(self, args):
        ...
        # spatial info
        self.vocab_spatial = ['on side of', 'next to', 'near', 'left', 'right','on the left of', 'on the right of', 'on the bottom of', 'above', 'on the top of','on top of', 'right of', 'left of', 'below'] #locality words
    ...
    def determine_position(self, locality, box1, box2, iou_threshold=0.1,distance_threshold=150):
        ...
        if locality in ['next to', 'on side of', 'near']:
        ...
        elif locality in ['on the right of', 'right of', 'right']:
            if x_distance < 0:
                ...
                score=0
        elif locality in ['on the left of', 'left of', 'left']:
            if x_distance > 0:
                if abs(x_distance) > abs(y_distance) and iou < iou_threshold:
                    score=1
                elif abs(x_distance) > abs(y_distance) and iou >= iou_threshold:
            ...
            if y_distance < 0:
                if abs(y_distance) > abs(x_distance) and iou < iou_threshold:
                    ...
                    score=iou_threshold/iou
        elif locality in ['on the top of', 'top of', 'above']:
            if y_distance > 0:
                if abs(y_distance) > abs(x_distance) and iou < iou_threshold:
                    score=1
                elif abs(y_distance) > abs(x_distance) and iou >= iou_threshold:

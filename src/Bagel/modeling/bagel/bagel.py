...
class Bagel(PreTrainedModel):
    ...
    def generate_text(
        ...
        while step < max_length:
            ...
            assert packed_query_sequence.requires_grad, "🚨 packed_query_sequence 没有梯度！"

            all_hidden_states.append(packed_query_sequence)
        ...
        return torch.stack([i.to(output_device) for i in generated_sequence], dim=0),all_hidden_states
    ...
    def chat(
        ...
        for k, v in generation_input.items():
            if torch.is_tensor(v):
                generation_input[k] = v.to(device)
        with torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16):
            unpacked_latent, _ = self.generate_text(
                past_key_values=past_key_values,
